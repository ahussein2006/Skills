...
public class TestWorkflow extends BaseWorkflow {
	...
	try {
	    ...
	    addWFTask(instance.getId(), getDelegate(2, processId), 2, curDate, taskUrl, WFTaskRolesEnum.SIGN_MANAGER, "1", null, requesterId);
	...
	try {
	    ...
	    Date curDate = MultiChronologyCalendarUtil.getSysDate(ChronologyTypesEnum.GREGORIAN);
	    completeWFTask(reTask, WFTaskActionsEnum.REVIEW.getValue(), curDate, getDelegate(2, instance.getProcessId()), 2, WFTaskRolesEnum.SIGN_MANAGER, "1", null, reTask.getAssigneeId());
	...
	try {
		...
		if (smTask.getOriginalId().equals(2L))
		    completeWFTask(smTask, WFTaskActionsEnum.SIGN.getValue(), curDate, getDelegate(3, instance.getProcessId()), 3, WFTaskRolesEnum.SIGN_MANAGER, "1", null, smTask.getAssigneeId());
		else
		...
		completeWFTask(smTask, WFTaskActionsEnum.RETURN_TO_REVIEWER.getValue(), curDate, getDelegate(instance.getRequesterId(), instance.getProcessId()), instance.getRequesterId(), WFTaskRolesEnum.REVIEWER_EMP, "1", null, smTask.getAssigneeId());
	    } else { // reject
